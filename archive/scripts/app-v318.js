...
function renderProjectCardHtml(p, geppoLabels) {
    ...
    return `
        <div class="project-card fade-in ${isSelectionMode ? 'selectable' : ''} ${isSelected ? 'selected' : ''}" data-id="${p.id}" oncontextmenu="return false;">
            ${isSelectionMode ? `
                <div class="card-selection-indicator ${isSelected ? 'active' : ''}">
                    ${isSelected ? '✓' : ''}
                </div>
            ` : ''}
...
function bindCardEvents(container) {
    container.querySelectorAll('.project-card').forEach(card => {
        ...
        const start = () => {
            ...
            longPressTimeout = setTimeout(() => { isLongPressAction = true; window.enterSelectionMode(id); }, 700);
        };
...
window.enterSelectionMode = async (firstId) => {
    ...
    window.isSelectionMode = true; // Hard Global
    if (firstId) selectedIds.add(firstId);
    
    const normalDock = document.getElementById('dock-normal');
    const selectionDock = document.getElementById('dock-selection');
    if (normalDock) normalDock.classList.add('hidden');
    if (selectionDock) selectionDock.classList.remove('hidden');
...
window.exitSelectionMode = async () => {
    ...
    window.isSelectionMode = false; // Hard Global
    selectedIds.clear();
    
    const normalDock = document.getElementById('dock-normal');
    const selectionDock = document.getElementById('dock-selection');
    if (normalDock) normalDock.classList.remove('hidden');
    if (selectionDock) selectionDock.classList.add('hidden');
...
function bindReportEvents() {
    ...
    if (els['btn-report-send']) {
        els['btn-report-send'].onclick = () => {
            ...
            window.location.href = `mailto:kmk7531.hmk.runner@gmail.com?subject=${subject}&body=${body}`;
